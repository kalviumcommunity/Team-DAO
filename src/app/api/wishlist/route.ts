...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: (error instanceof Error ? error.message : "Unknown error") || "Failed to fetch wishlist" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: (error instanceof Error ? error.message : "Unknown error") || "Failed to add to wishlist" },
      { status: 400 }
    );
  }
}
